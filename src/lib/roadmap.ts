...
export const ROADMAP: RoadmapItem[] = [
  ...
  { title: "Create the custom_requests + email_campaigns + client_profiles tables in Supabase", priority: 1, status: "todo", needs: "run the SQL blocks at the end of supabase/schema.sql", detail: "custom_requests powers personalized extra work + its payment link; email_campaigns records broadcast history; client_profiles stores the client's photo, name, phone and marketing opt-in. All three warn (but don't break) until the tables exist." },
  { title: "Create the page_views table in Supabase", priority: 1, status: "todo", needs: "run the page_views block at the end of supabase/schema.sql", detail: "Powers /admin/traffic (servolia.com visitors) AND the portal's Visitors tab (each client's own site traffic). Both pages show a 'table doesn't exist yet' state until this is run — nothing breaks, but no traffic is recorded either." },
  ...
  { title: "Decide whether to drop the Meta Pixel entirely", priority: 3, status: "todo", detail: "The paid-ads OFFERING is retired (July 2026): the Ads Landing plan is marked retired in pricing.ts and every Meta Ads claim is off the public pages. But the pixel itself still fires site-wide via src/components/Analytics.tsx, and /legal/terms still discloses it (correctly). If you're never running Meta ads again, remove the pixel + metaCapi and update the terms in the same change." },
