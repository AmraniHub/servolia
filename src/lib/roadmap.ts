...
export const ROADMAP: RoadmapItem[] = [
  ...
  { title: "French versions of /solutions/[slug] and /niches/*", priority: 3, status: "in_progress", detail: "10 SEO/funnel pages with no French equivalent — pure missed organic reach in the beachhead market. FIRST TRANCHE SHIPPED 2026-07-27: /fr/[niche]/[ville] (dentiste, clinique-esthetique, services-a-domicile × 15 top FR cities = 45 geo pages) + /fr/villes hub. Uses FAQPage + LocalBusiness/Service JSON-LD so LLMs (Perplexity, Google AI Overviews, ChatGPT) can lift the answers. STILL TODO: French /solutions/[slug] equivalents, plus a translation of the English /niches/* long-form content." },
  ...
  { title: "Pay-per-booking pricing — aesthetic/med-spa pilot unblocked, dental/medical stays blocked", priority: 1, status: "in_progress", needs: "run the pay-per-booking SQL block at the end of supabase/schema.sql, then build the checkout + invoicing cron", detail: "Resolved 2026-07-25: this was rotting as a single blanket 'blocked' line. It's now two separate things. (1) src/lib/pricing.ts defines PAY_PER_BOOKING (€990 setup + €60/attended consultation) and payPerBookingEligible(niche) — a hard gate that returns false for anything dental/medical-shaped and only true for aesthetic/med-spa/beauty. That's usable today for aesthetic clients. (2) Dental/medical stays blocked behind a French lawyer sign-off on 'compérage' rules (Ordre des Chirurgiens-Dentistes/Médecins) — do not call payPerBookingEligible for a dental niche true, and don't relax the regex without that legal review. Schema added 2026-07-25 (clients.billing_mode/per_booking_rate_eur, chat_sessions.billed_at, pay_per_booking_invoices table) — not yet run. STILL TODO before this earns real revenue: a checkout path for the setup fee (today's build-plan checkout assumes a 50% deposit against a flat total, which doesn't fit setup+per-result billing) and a monthly cron that tallies unbilled attended bookings per pay-per-booking client, writes a pay_per_booking_invoices row, and creates the actual Stripe invoice — same shape as /api/cron/monthly-report, but invoicing instead of just reporting." },
  ...
  { title: "Google Places API — one-click prospect ingestion from a Google Maps URL", priority: 2, status: "todo", needs: "GOOGLE_PLACES_API_KEY", detail: "Today's prospect adder is: paste CSV, or type one row manually. Real workflow the founder wants: paste a Google Maps link, we call Places Details, we populate business + phone + address + website + hours automatically. Then the existing 'Generate demo' + 'Draft email' buttons take over. Also unlocks bulk-import by search (e.g. 'aesthetic clinic Lyon' → 20 prospects). Costs $0.017 per Details call — trivial for the pipeline it feeds." },
