...
export const FEATURES: SystemFeature[] = [
  ...
  },
  {
    ...
    how: [
      "ESTABLISHED CLIENTS (owner's rule: existing subscribers' records are not changed): a real row created before SETUP_TRACKER_SINCE (2026-09-25T00:00:00Z, src/lib/hostingSetup.ts) — or with no date at all — gets no checklist on their page (plan and measured status only, as before), no write to their row, no setup email, no founder notice, and the cron never picks them up. /admin/hosting/<id> shows the checklist read-only with 'Established client — tracker not used'. Nothing is backfilled. Test rows are never established.",
      ...
      "WHAT IS FETCHED: the client's address is fetched only when every address it resolves to is public. Redirects are followed by hand, at most two hops, each https and on the same registrable domain; a redirect to 169.254.169.254, localhost, plain http or another domain stops the check and is never requested. The Vercel API calls are capped at 4 seconds.",
      ...
      "AUTOMATIC RE-CHECKS: the quarter-hourly /api/cron/domain-live gives its leftover time (max 15 s) to recheckHostingSetups(): every new hosting row whose setup is not complete, oldest check first, four at a time. 'Check again' on the portal runs the same check; it and the page's own re-measure of a stale check share one limit (4 per subscription per 10 min; 20 per IP per hour for the button).",
      "EMAILS, ONCE AND TRUE: nothing is news until a check has actually MEASURED the address (no resolver error and a real HTTP status) — a failed first probe never sets the starting point. The first measured check is a silent baseline; after it, a milestone reached is CLAIMED on the row (compare-and-swap on setup.rev, so racing checks send one email), sent, and only then stamped with the send time. A failed send is stamped 'failed' and retried at the next check, up to 5 times; a crashed send's claim is taken again after 10 minutes. Domain and live together send only the live email. Templates: src/lib/hostingSetupEmails.ts, each with the service-page link. Your notice goes through notifyOwner (src/lib/notify.ts: Telegram + OWNER_ALERT_EMAIL). Every send — the client's email and your notice — is awaited and capped at 5 s with bounded(). A client email Resend refused is retried; one with no answer in time is UNCONFIRMED (emailOutcome) and never retried, because it may already be in their inbox; your notice says which, and tells you to check Resend.",
