...
export const FEATURES: SystemFeature[] = [
  ...
  },
  {
    ...
    how: [
      ...
      "DOMAIN ONLY (src/lib/domainOrders.ts): /admin/hosting → New domain link → domain, client email, name, Vercel project, language. The server quotes the name live and makes a LIVE Stripe Checkout link (expires in 23h; always live, like every admin link, whatever test mode says). The endpoint needs the admin session AND a same-origin request (src/lib/sameOrigin.ts). The client pays with his own card; the card is saved for next year.",
      "THE WEBHOOK (kind domain_order): first writes a CLAIM on the Stripe customer, then buys the name on Vercel (WHOIS privacy, auto-renew on), checking the money that arrived covers the price (in USD, Adaptive Pricing included). Vercel registers ASYNCHRONOUSLY: the webhook reads the order for ~4 seconds; completed → attaches apex + www to the named project and emails 'registered'; still purchasing → records 'purchasing' and emails 'being completed'; failed → emails 'finished by hand or refunded'. Telegram either way. It has its own try/catch that Telegrams you: the webhook's outer catch answers 200, so Stripe never retries a throw.",
      "NEVER BOUGHT TWICE: a replayed event finds the record on the customer and does nothing (no email, no alert). A replay that finds only the CLAIM means the first run died mid-purchase: nothing is bought, you get a Telegram to check Vercel > Domains, and the client is not emailed. A founder TEST purchase never reaches the registrar (twice guarded: the webhook's test flag and registrar() itself).",
      "PURCHASING → REGISTERED: /api/cron/domain-orders (every 15 min, 7/22/37/52) reads each order still purchasing; completed → attach + 'registered' email + Telegram; failed → marked failed + email + Telegram; still purchasing 6h after payment → one Telegram.",
      "The record of a domain-only order lives on the STRIPE CUSTOMER's metadata (servolia_domain, _status claimed/purchasing/bought/failed/stopped, _retail, _renews, _project, _attached, _order, _session, _noticed, _noticed_usd...), one customer per order. No table, no migration, and it never counts as a hosting client.",
      "Its renewal (/api/cron/domain-billing): 30 days before, a price-rise notice email if the price is going up (recorded with the price it announced); 7 days before, a one-line invoice charged on the saved card. THE CHARGE NEVER EXCEEDS WHAT THE CLIENT WAS TOLD: the noticed price, or last year's when no notice went out, and Telegram shows the margin held back. The invoice is found again by its metadata on a retry, so a year is never charged twice; a draft left empty by a crashed run gets its line, and an invoice 'paid' for 0 never counts. A declined card goes to Telegram daily until it is resolved: Vercel auto-renews on OUR card either way. Every renewal email that fails to send is on the Telegram too.",
    ...
    use: [
      "Selling a domain to someone with no plan (first: Ithar Digital, ithardigital.com → ithar-digital): make the link, send it to him, he pays. Watch Telegram for 'REGISTERED' and 'Attached to Vercel project' (or 'still registering', then the 15-minute cron's 'now REGISTERED').",
      ...
      "Telegram says 'NOT registered' or 'order FAILED': buy it by hand (vercel domains buy <name>) or refund the payment in Stripe. The client was already told one or the other happens.",
      "Telegram says 'NOT attached': Vercel > the project > Domains > add it.",
      "Telegram says 'purchase was claimed and never recorded': look in Vercel > Domains. There → set servolia_domain_status to 'bought' (and _renews to a year after purchase) in Stripe and email the client; not there → buy it or refund.",
    ],
    cost: "Vercel's price for the name (~$11.25 for a .com), Stripe's fee. About $13 kept per domain per year, by construction. The settle cron is one Stripe search every 15 minutes.",
    ...
    code: "src/lib/domainSales.ts · src/lib/domainOrders.ts · src/lib/sameOrigin.ts · /api/admin/domain-order · src/components/admin/DomainOrderLink.tsx · webhook kind domain_order · /api/cron/domain-orders · /api/cron/domain-billing · tests/domain-orders.test.mjs",
  },
  {
