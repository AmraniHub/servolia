...
export const FEATURES: SystemFeature[] = [
  ...
  },
  {
    ...
    how: [
      ...
      "A DOMAIN WE ALREADY OWNED, sold with a hosting link (src/lib/ownedDomain.ts, note `servolia-owned-domain:`): only accepted when Vercel shows it was registered through our team (a purchase date). Each year the domain-billing cron renews it by the same rules, as a line pinned to THE HOSTING SUBSCRIPTION's next invoice (tagged owned_domain_renewal), and moves the note's date on (`billed:` records the year just billed). The renewal date follows Vercel's actual expiry when readable, except while a billed year has not been renewed by Vercel yet. When that hosting ENDS (customer.subscription.deleted): unbilled renewal lines are deleted; Vercel auto-renew goes off — unless a paid year has not started, then it stays on until that date and the daily cron switches it off after; the cancellation notice names the domain and what was done. A founder test cancellation never touches Vercel.",
