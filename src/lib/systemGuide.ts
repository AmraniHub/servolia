...
export const FEATURES: SystemFeature[] = [
  {
    ...
    how: [
      ...
      "Excluded everywhere with 'is_test is not true' (so every pre-existing row still counts): crm_kpis, the dashboard, /admin/revenue, the data room, economics, /admin/today, the monthly invoice/overage, monthly report, dunning and domain-billing crons, and the conversation-cap alert to you. /admin/clients and /admin/hosting still LIST test rows, marked TEST, but never total them.",
      "Before supabase/2026-09-24-test-mode.sql has run there is no is_test column: the webhook refuses every test event whole (nothing written, Telegram tells you), and every query runs exactly as it did before.",
    ],
    use: [
  ...
  },
  {
    ...
    summary: "Any domain we sell is priced from Vercel's renewal price, never under $27.90 a year and always ending .90 when it is SOLD; at each renewal it moves only when the registry raised Vercel's price enough to eat the margin, with 30 days' notice. A client with no Servolia plan can buy one on its own from a link made on /admin/hosting and gets a Servolia email.",
    how: [
      "THE PRICE (his rule, 2026-09-24): retail = (Vercel renewal + $13 profit + Stripe's worst case), rounded UP to the next .90, floor $27.90, ceiling $80 (above that it is not offered). A .com (Vercel ~$11.25) sells at $27.90. src/lib/domainSales.ts retailYearlyUsd. The floor and the rounding apply to NEW sales only.",
      "RENEWALS MOVE ONLY WHEN THE REGISTRY DID (renewalRetailUsd): last year's price, unless Vercel's current renewal price leaves under $13 at it — then the lowest .90 that restores $13. A plan client who bought at $26 before the floor keeps $26 while the margin holds. Never down. Unknown Vercel price = last year's.",
      "THE 30-DAY RULE (renewalDecision, shared by every domain): the promise is 30 days before the MONEY moves, so a rise is emailed between 44 and 37 days before the renewal date (the card is charged at −7) and never later, and recorded ONLY once the email went (a failed send is retried the next day while the window lasts). The charge, 7 days before the date, never exceeds what the client was told: the announced price, or last year's when no notice went out — Telegram shows the margin held back, and next year's notice carries it. Applies to a monthly plan's domain and panel add-ons (invoice line on their next plan invoice) and to domain-only orders (card). An ANNUAL plan's domain is a fixed line on its Stripe subscription: the margin watch sends a Telegram once when its profit drops $3 under target, and you reprice it by hand.",
      "A DOMAIN WE ALREADY OWNED, sold with a hosting link (src/lib/ownedDomain.ts, note `servolia-owned-domain:`): only accepted when Vercel shows it was registered through our team (a purchase date) and stays registered at least ~11 months (a 'first year' must be a year). Each year the domain-billing cron first asks Vercel: a domain no longer in our team is never billed (reported once), and a day Vercel cannot be read bills and announces nothing. It then reads the hosting subscription: cancelled at period end, set to end, or already ended (a missed deletion webhook) means NOTHING is billed — the owner is told once ('<client> cancelled — domain renewal NOT charged; decide: renew at our cost, let it lapse, or ask the client'), the note records `sub-ending:`, and Vercel auto-renew goes off unless a paid year is still to start or the hosting itself runs past the domain's expiry (then it stays on until the hosting ends); if the client takes the cancellation back, the markers clear and auto-renew comes back on — unless the domain expired meanwhile, which is reported for a renewal by hand and nothing is billed. A tagged line found on ANOTHER invoice (the hosting's) counts as billed there, at the line's own amount: the domain path never pays or stamps an invoice that is not its own. Then it renews by the same rules — on an INVOICE OF ITS OWN charged on the charge day on the subscription's card when the hosting's next invoice falls after the renewal date (Ithar: domain 2027-09-25, hosting 2027-10-09; auto_advance off, tagged, found again by its tag, at most 4 declined attempts counted on the note), otherwise as a line pinned to THE HOSTING SUBSCRIPTION's next invoice (every renewal line is tagged and looked for first, so a failed row write never leads to a second line), and moves the note's date on (`billed:` records the year just billed). The next year is NEVER billed until Vercel has actually renewed the last one (expiry after `billed`); 5 days late, Telegram says 'paid year NOT renewed by Vercel — renew by hand or refund' every day. Vercel's expiry is followed only within 30 days of our date; a bigger gap is reported once (`vercel-expiry:`) and our date stands. When that hosting ENDS (customer.subscription.deleted): unbilled lines and lines on draft invoices for a year not yet started are deleted, while an unbilled line whose year HAS started is invoiced on its own and charged on the card (a decline is reported); an unpaid invoice carrying only the renewal of a year not started is voided (one mixing other charges is reported, credit that line by hand); Vercel auto-renew goes off — unless a paid year has not started, then it stays on until Vercel has renewed that year and the daily cron switches it off after. If the client comes back (invoice.paid on the ended row), auto-renew is switched back on and the markers cleared. The cancellation notice names the domain and what was done. A founder test never touches Vercel.",
      "DOMAIN ONLY (src/lib/domainOrders.ts): /admin/hosting → New domain link → domain, client email, name, Vercel project, language. Refused for an ending outside .com .org .net .co .uk .io (.fr and .ma have registry eligibility rules a US LLC registrant may not meet), a Vercel project that does not exist in the team, or a name Vercel will not sell. The server quotes the name live and makes a LIVE, card-only Stripe Checkout link (expires in 23h; always live, like every admin link). The endpoint needs the admin session AND a same-origin request (src/lib/sameOrigin.ts). The card is saved for next year.",
      "THE WEBHOOK (kind domain_order): writes a CLAIM on the Stripe customer with the idempotency key domain-claim-<session> — atomic, so two deliveries at once make ONE registrar call (the second gets a replay or a 409 and says nothing). Then it checks the money covers the price in USD (Adaptive Pricing read from currency_conversion; an amount it cannot read buys nothing), buys on Vercel, and reads the order for ~4 s. completed → attach apex + www, email 'registered'; purchasing → email 'being completed'; refused → 'failed', email 'finished by hand or refunded'; the buy call died (network/5xx) → 'unknown': it MAY have gone through, so nobody refunds or re-buys before looking at Vercel. A quote that fails is 'could not check', never 'taken'. The owner gets a notifyOwner notice (Telegram + email): '💶 Paid: Domain <name>' or '⚠️ Domain NOT registered / purchase unknown / order interrupted / conflict', with a Next: line and the exact way to close it. Every send awaited and bounded (src/lib/notify.ts); its own try/catch, because the webhook's outer catch answers 200 and Stripe never retries a throw.",
      "NEVER BOUGHT OR SAID TWICE: a replay finds the record and sends nothing at all. A replay that finds only the CLAIM (first run died mid-purchase) buys nothing and tells you; the client is not emailed. A founder TEST purchase never reaches the registrar (the webhook's test flag and registrar() itself).",
      "PURCHASING → REGISTERED: /api/cron/domain-orders (every 15 min, 7/22/37/52) settles each order still purchasing: completed → attach + 'registered' email + Telegram; failed → marked failed + email + owner notice; still purchasing 6h after VERCEL ACCEPTED IT (placedAt) → reported once. The record is written before the email, so no run sends a second one.",
      "The record lives on the STRIPE CUSTOMER's metadata (servolia_domain, _status claimed/purchasing/bought/failed/unknown/stopped, _retail, _renews, _bought, _project, _attached, _order, _session, _claimed, _placed, _noticed, _noticed_usd, _renewal_off, _note), one customer per order. No table, no migration, never a hosting client.",
      "Its renewal: a one-line invoice with auto_advance OFF — the daily cron is its only collector, one attempt a day, at most 4, the count kept on the CUSTOMER first (then the invoice), so a failed invoice update cannot lose one; after the 4th decline it says 'STOPPED RETRYING' once. A record with no stored price is never billed at a guessed figure: it is reported daily until someone sets it. The invoice is found again by its metadata, so a year is never charged twice; a draft still empty after its line was added is DELETED, never finalised at 0, and an invoice 'paid' for 0 never counts.",
      "DAILY AUDIT (in the domain-billing Telegram, so at most once a day): a bought record with no renewal date; a failed/unknown record whose name IS in our Vercel team (a domain we pay for that nobody is charged for); a claim older than an hour. And the stopped sweep: any stopped order whose renewal invoices are not voided or whose Vercel auto-renew is not off.",
    ...
    use: [
      "Selling a domain to someone with no plan: make the link, send it, they pay. Watch for '💶 Paid: Domain <name>' (registered, or still registering and then the 15-minute check's 'now REGISTERED').",
      "Finishing an order by hand (NOT registered / unknown / interrupted / stuck): buy it if Vercel > Domains does not have it, then /admin/hosting → New domain link → Existing order → the domain → Mark bought. It refuses unless Vercel shows we BOUGHT the name through Vercel (a purchase date, not older than the order — a name merely added to the team does not count) and the client's payment was not refunded; then it dates the renewal a year after Vercel's purchase date, attaches it to the recorded project and emails the client 'registered'. If several Stripe customers hold the name, give the cus_ id.",
      "A client asks to stop renewing: Existing order → Stop renewing (two clicks; bought orders only — the others say what to do instead). Draft renewal invoices deleted, open ones voided, Vercel auto-renew switched off — EXCEPT when the next year is already paid: then auto-renew stays on until that date (so Vercel renews what the client bought) and the daily sweep switches it off after. Setting servolia_domain_status=stopped by hand in Stripe is caught by the daily sweep, which does the rest.",
      "Telegram says 'NOT attached': Vercel > the project > Domains > add it.",
      "Telegram says 'STOPPED RETRYING': get a new card from the client (Stripe > customer > payment method, then pay the open invoice), or stop the renewal.",
    ],
    cost: "Vercel's price for the name (~$11.25 for a .com), Stripe's fee. About $13 kept per domain per year, by construction. The settle cron is one Stripe search every 15 minutes; the daily audit a few searches and one Vercel read per failed order.",
    value: "Clients who are not on a plan can still buy their address through Servolia, in two minutes and with their own card. Each one is recurring profit every year, and the profit holds when the registry raises its prices — without ever surprising the client.",
    code: "src/lib/domainSales.ts · src/lib/domainOrders.ts · src/lib/sameOrigin.ts · /api/admin/domain-order (+ /manage) · src/components/admin/DomainOrderLink.tsx · webhook kind domain_order · /api/cron/domain-orders · /api/cron/domain-billing · tests/domain-orders.test.mjs · tests/domain-billing-cron.test.mjs · tests/webhook-alerts.test.mjs (MONEY)",
  },
  {
  ...
  },
  {
    ...
    how: [
      ...
      "THE CLIENT EMAIL'S REAL OUTCOME IS IN THE ALERT: on a payment failure (plan and hosting) and a multilingual order the client email is awaited first, and the owner is told 'sent', 'FAILED — tell/email them by hand' (Resend refused), or 'NOT CONFIRMED — check it went' (no answer within the 5 s cap: it may have gone, so it is never called a failure).",
      "EVERY CHECKOUT IS CARD-ONLY (payment_method_types card, all ten creators): a delayed method (SEPA, bank transfer) would complete the session unpaid, and the webhook fulfils only a PAID session — checkout.session.async_payment_succeeded is deliberately not handled, so card-only is what keeps a buyer from paying and receiving nothing.",
      "THE CHAT WIDGET NEVER MAKES A PATIENT WAIT FOR ALERTS: the clinic owner's lead alert and the client-pixel Meta Lead run in after() (on Vercel, waitUntil keeps the function alive until they settle, within the route's maxDuration), and only on the conversation's FIRST booking, so one patient is one Lead.",
      "ONE-OFF WORK IS RECORDED WITH ITS DUE DATE (src/lib/oneOffOrders.ts): a paid multilingual search setup writes a `servolia-oneoff:` line (service, session, paid, due = five working days, amount) on the client's hosting_clients row — matched by the ref's repository first, then by the buyer's address (exact, case-insensitive), an active row preferred — or, with no row, a lead with source 'one-off' and stage 'one_off' (not pipeline, not an installation won). Keyed on the checkout session and checked in BOTH stores first: a redelivered event stops before any email or alert, even after the client has since bought hosting. The hosting setup form keeps recorded orders when it saves the notes. /admin/today shows 'Multilingual search setup for <site> — due <date>' (today when overdue or due within a day) until Done is pressed (POST /api/admin/oneoff-done). The owner email and Telegram name the due date. A test purchase matches only test rows and tags its lead is_test.",
  ...
  },
  {
    ...
    summary: "Every NEW hosting client (Essential, Complete, Business, created on or after 2026-09-25) sees a checklist at the top of their service page: paid, site details, set up on our servers, domain pointed, https, live, forms tested (and the mailbox on Business). Auto steps are measured; hand steps are yours to tick. The client is emailed once when their domain points to us and once when the site is live; you are told once too. Clients from before that date are ESTABLISHED: the tracker is not used for them at all.",
    how: [
      "ESTABLISHED CLIENTS (owner's rule: existing subscribers' records are not changed). A real row is established when it was created before SETUP_TRACKER_SINCE (2026-09-25T00:00:00Z) or has no date, OR its email is a known client reference (CLIENT_REFS), OR the same email (case-insensitive, exact) already has an older real row from before the cutover — an existing client who checks out again gets a fresh row dated today and must still be left alone (establishedRow in src/lib/hostingSetupRun.ts; one query per address, cached per request; a lookup that fails counts as established). For them: no checklist on their page (plan and measured status only), no write to their row, no setup email, no founder notice, the cron never picks them up, and /admin/hosting/<id> shows the checklist read-only with 'Established client — tracker not used'. Nothing is backfilled. Test rows are never established.",
      ...
      "AUTO (measured): Paid = the row. Site details = the setup form arrived (stamp, or the 'Platform:' line it writes in notes; a client already in CLIENT_REFS or with a repo recorded needs no form). Domain pointed = a live DNS lookup that every required record answers from Vercel (A in Vercel's ranges, CNAME to vercel-dns, or Vercel nameservers). https = a TLS handshake with a verified certificate. Live = HTTP 200 over https with Vercel's own headers, on their own host, AND Vercel's API confirming the host is on the Vercel project recorded for them — headers alone would also pass for somebody else's Vercel account (src/lib/hostingSetupProbe.ts).",
      "HAND (ticked by you): 'Your site set up on our hosting' — it can only be ticked once the Vercel project is recorded in Hosting setup on the same page, and stops counting if the project is cleared; 'Contact forms (and tracking) tested'; on Business 'Business email on your domain'. The client sees 'We're doing this' with the date it started. The onboarding step carries the only timeframe promised in writing (SetupForm: we get in touch within one working day), always beside the date it runs from; /admin/today's 'needs-setup' row says OVERDUE once a working day has passed (onboardOverdue). No other timeframe is promised or shown.",
      ...
      "WHAT IS FETCHED: nothing — no HTTP request, no TLS handshake — until DNS points the client's address at Vercel. Every connection then resolves the name itself and refuses any non-public A or AAAA answer (publicOnlyLookup: DNS pinned, so a rebinding server gets no second answer), for the first request and for every redirect hop. Redirects are followed by hand, at most two hops, each https, port 443 and on the same registrable domain; a redirect to 169.254.169.254, localhost, plain http or another domain stops the check and is never requested. The Vercel API calls are capped at 4 seconds.",
      ...
      "AUTOMATIC RE-CHECKS: the quarter-hourly /api/cron/domain-live gives its leftover time (max 15 s) to recheckHostingSetups(): every new hosting row whose setup is not complete, oldest check first, four at a time. 'Check again' on the portal runs the same check (4 per subscription per 10 min, 20 per IP per hour). Opening the page may re-measure a stale check under its OWN limit (6 per subscription per 10 min), so it never uses up the button.",
      "EMAILS, ONCE AND TRUE: for a NEW client every milestone is news — including one already reached at the very first check, so a client whose site only became measurable once live still gets the live email. A milestone reached is CLAIMED on the row (compare-and-swap on setup.rev, so racing checks send one email), sent, and only then stamped with the send time. Resend refused = 'failed', retried at the next check, up to 5. No answer inside the 5 s cap = 'unconfirmed' (alerts-fix's emailOutcome): it may have gone, so it is never retried. A claim still standing after 10 minutes (the process died mid-send) is also settled 'unconfirmed', never resent, and you are told once. Domain and live together send only the live email. Templates: src/lib/hostingSetupEmails.ts, each with the service-page link. Your notices go through notifyOwner (src/lib/notify.ts), every send awaited and bounded.",
      ...
      "WAY BACK IN: the sign-in screen on /hosting/account has 'Email me my link' (POST /api/hosting-account/link { email }). Same answer and same timing for a client and a stranger (the lookup and send run after the response), only the address on the row is emailed. Its limit — 5 per IP and 3 per address per hour — is ATOMIC (one Postgres statement, servolia_rate_hit, src/lib/atomicLimit.ts) and FAILS CLOSED: until the SQL has run it refuses every request.",
      "NEEDS supabase/2026-09-25-hosting-setup.sql (one nullable jsonb column, the servolia_rate_hit function, and row level security on rate_limits with no policies — only the service-role server touches that table). Until it runs: the checklist still shows for new clients (measured on each visit) but nothing is stored, no milestone email is sent, ticks answer 'run the SQL first', the cron Telegrams you once a day, and 'Email me my link' refuses.",
    ],
    use: [
      "Run supabase/2026-09-25-hosting-setup.sql once. Nothing needs doing for existing clients: the tracker leaves them alone.",
      "A NEW client: /admin/hosting/<id> → record the Vercel project in Hosting setup, then 'Setup checklist' → 'Mark done' on 'on our hosting'; the rest measures itself. 'Run checks now' measures immediately; tick forms (and the mailbox on Business) when true.",
    ...
    cost: "None beyond the existing cron: a few DNS lookups, one TLS handshake, one HTTP request and one Vercel API call per incomplete new client per quarter hour.",
