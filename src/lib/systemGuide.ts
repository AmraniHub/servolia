...
export const FEATURES: SystemFeature[] = [
  ...
  },
  {
    ...
    how: [
      ...
      "Its renewal: a one-line invoice with auto_advance OFF — the daily cron is its only collector, one attempt a day, at most 4, the count kept on the invoice; after the 4th decline it says 'STOPPED RETRYING' once. The invoice is found again by its metadata, so a year is never charged twice; a draft still empty after its line was added is DELETED, never finalised at 0, and an invoice 'paid' for 0 never counts.",
    ...
    use: [
      ...
      "Finishing an order by hand (NOT registered / unknown / interrupted / stuck): buy it if Vercel > Domains does not have it, then /admin/hosting → New domain link → Existing order → the domain → Mark bought. It refuses unless Vercel shows the name in our team, dates the renewal a year after Vercel's purchase date, attaches it to the recorded project and emails the client 'registered'.",
      "A client asks to stop renewing: Existing order → Stop renewing (two clicks). Draft renewal invoices deleted, open ones voided, Vercel auto-renew switched off. Setting servolia_domain_status=stopped by hand in Stripe is caught by the daily sweep, which does the rest.",
