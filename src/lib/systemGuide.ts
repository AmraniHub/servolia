...
export const FEATURES: SystemFeature[] = [
  ...
  },
  {
    ...
    how: [
      ...
      "The tiers differ by VOLUME, not by whether the AI receptionist exists — every plan includes it. Going over the included conversations moves them to the next tier rather than billing surprise overage — but BY HAND: the monthly-invoice cron only flags it on Telegram, and you change the plan after telling the client. Predictable for them, expansion revenue for you.",
  ...
  {
    name: "Client domains — she keeps hers, her site goes on it (C2)",
    summary: "A practice keeps her domain at her own registrar and stays its owner (CGV 7 bis). Servolia attaches it to the site and tells her which DNS lines to add; the site goes live on it, and she is told, the first time it answers.",
    how: [
      "THE DECISION (2026-09-22, his): Vercel's registrar refuses .fr, .be, .eu and .lu, which is most of the market. So Servolia does not buy French domains: she keeps hers at OVH, IONOS, Gandi or wherever it is, and remains the registrant. A new .com can still be bought through Vercel (src/lib/domainSales.ts) when a client has no domain at all.",
      "Attaching: /admin/sites → Attach on the site card. src/lib/siteDomain.ts adds the apex and a www redirect to this Vercel project and returns the DNS lines (A @, CNAME www, and a TXT when Vercel wants proof of ownership) straight from Vercel's own recommendation.",
      "Serving: src/proxy.ts maps her Host to her site (src/lib/siteHost.ts) — her pages at the root, her robots.txt and sitemap.xml, the chat on /api/site-chat, 404 for anything of servolia.com. Her page names Servolia only on its privacy page, as processor.",
      "Going live: /api/cron/domain-live (every 15 min) fetches https://<her domain>/ and, the first time it serves THIS site (the site-id marker), stamps domainLiveAt and sends the go-live email with her address in it. From then on servolia.com/sites/<slug> 308s to her domain.",
      "The Cloudflare registrar stack (src/lib/domains.ts, the client_domains table, the Monday domain-renewals cron) was removed that day: it was never enabled, no code wrote to its table, and the portal panel that read it now reads her site's config.",
    ],
    use: [
      "Attach her domain on /admin/sites, copy the DNS lines into an email to her (or her webmaster). That is the whole manual part.",
      "Today lists a domain that has not answered 24h after attaching. It is almost always a DNS line not added, or added at the wrong place.",
      "Detach puts the site back on servolia.com/sites/<slug>.",
    ],
    cost: "Nothing for a domain she already owns. Whether the plan still pays her renewal (CGV 7 bis says one standard domain's fees are included) is an open decision for him.",
    value: "A practice's own address, in her name, with no agency holding it — the thing a clinic burned by a previous web agency needs to hear.",
